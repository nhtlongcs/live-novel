...
parser.add_argument(
    "-i", type=int, help="Number of steps", default=250, dest="max_iterations"
)
parser.add_argument(
    "-s",
    nargs=2,
...
parser.add_argument(
    "-ii", type=str, help="Initial image", default=None, dest="init_image"
)
parser.add_argument("-ss", type=int, help="Skip steps", default=-1, dest="skip_steps")
parser.add_argument(
    "-cuts", type=int, help="Number of cut batches", default=4, dest="cutn"
)
...
parser.add_argument(
    "-dvitb32",
    type=str,
    help="Use VitB32 CLIP model? yes/no",
    default="yes",
    dest="VitB32",
)
parser.add_argument(
    "-dvitb16",
    type=str,
    help="Use VitB16 CLIP model? yes/no",
    default="yes",
    dest="VitB16",
)
parser.add_argument(
    "-dvitl14",
    type=str,
    help="Use VitL14 CLIP model? yes/no",
    default="no",
    dest="VitL14",
)
parser.add_argument(
    "-drn101", type=str, help="Use RN101 CLIP model? yes/no", default="no", dest="RN101"
)
parser.add_argument(
    "-drn50", type=str, help="Use RN50 CLIP model? yes/no", default="yes", dest="RN50"
)
parser.add_argument(
    "-drn50x4",
    type=str,
    help="Use RN50x4 CLIP model? yes/no",
    default="no",
    dest="RN50x4",
)
parser.add_argument(
    "-drn50x16",
    type=str,
    help="Use RN50x16 CLIP model? yes/no",
    default="no",
    dest="RN50x16",
)
parser.add_argument(
    "-drn50x64",
    type=str,
    help="Use RN50x64 CLIP model? yes/no",
    default="no",
    dest="RN50x64",
)
                            ...
                            if args.steps_per_checkpoint is not None:
                                if j % args.steps_per_checkpoint == 0 and j > 0:
                                    if args.intermediates_in_subfolder is True:
                                        ...
                                        im.save(
                                            (partialFolder + "/" + filename).replace(
                                                ".png", ".jpg"
                                            ),
                                            quality=80,
                                        )
...
if ViTB32 is True:
    clip_models.append(
        clip.load("ViT-B/32", download_root="./content/clips/", jit=False)[0]
        .eval()
...
if ViTB16 is True:
    clip_models.append(
        clip.load("ViT-B/16", download_root="./content/clips/", jit=False)[0]
        .eval()
        .requires_grad_(False)
        .to(device)
    )
if ViTL14 is True:
    clip_models.append(
        clip.load("ViT-L/14", download_root="./content/clips/", jit=False)[0]
        .eval()
        .requires_grad_(False)
        .to(device)
    )
if RN50 is True:
    clip_models.append(
        clip.load("RN50", download_root="./content/clips/", jit=False)[0]
        .eval()
        .requires_grad_(False)
        .to(device)
    )
if RN50x4 is True:
    clip_models.append(
        clip.load("RN50x4", download_root="./content/clips/", jit=False)[0]
        .eval()
        .requires_grad_(False)
        .to(device)
    )
if RN50x16 is True:
    clip_models.append(
        clip.load("RN50x16", download_root="./content/clips/", jit=False)[0]
        .eval()
        .requires_grad_(False)
        .to(device)
    )
if RN50x64 is True:
    clip_models.append(
        clip.load("RN50x64", download_root="./content/clips/", jit=False)[0]
        .eval()
        .requires_grad_(False)
        .to(device)
    )
if RN101 is True:
    clip_models.append(
        clip.load("RN101", download_root="./content/clips/", jit=False)[0]
        .eval()
        .requires_grad_(False)
        .to(device)
    )
...
if iargs.max_iterations and iargs.max_iterations >= 250:
    ...
    intermediate_saves = [
        iargs.max_iterations - 100,
        iargs.max_iterations - 50,
        iargs.max_iterations - 30,
        iargs.max_iterations - 10,
    ]  # @param{type: 'raw'}
else:
